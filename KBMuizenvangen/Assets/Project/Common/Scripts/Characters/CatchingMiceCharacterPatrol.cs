...
public class CatchingMiceCharacterPatrol : ICatchingMiceCharacter {
	...
	public List<Waypoint> patrolRoute = new List<Waypoint>();
	...
	protected IEnumerator PatrolRoutine()
	{
		...
		{
			Waypoint current = patrolRoute[waypointIndex];
			Waypoint next = null;
			...
			bool fullPath = false;
			List<Waypoint> path = CatchingMiceUtil.FindPath(navigationGraph, current, next, out fullPath, walkable);
